<script lang="ts">
	...
	const createTodo = (text: string, done = false, shape: string) => ({
		id: ++lastId,
		text,
		done,
		shape,
	...
	let todos = [
		createTodo('Learn Svelte', true, 'box'),
		createTodo('Learn Threlte', false, 'sphere')
	]

	const total = tweened(todos.length)

	$: {
		total.set(todos.length)
	}
...
<InstancedMesh position.y={-5}>
	...
	{#each todos as todo, index (todo.id)}
		<Instance transition={scale} position.y={-(3.5 * ($total-index-1))} rotation.y={degToRad(0)}>
